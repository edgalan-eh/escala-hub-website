      ...
      <div className="container pt-20 pb-10">
        <div className="grid gap-12" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))" }}>
          <div className="flex flex-col items-start gap-5" style={{ gridColumn: "span 2" }}>
            <Image src="/brand/escala/logo-escalahub-dark.svg" alt="Escala Hub" width={160} height={29} style={{ height: 28, width: "auto" }} />
          ...
          <div className="flex flex-col items-start gap-2.5">
            ...
            <a href={site.whatsapp} target="_blank" rel="noopener" className="t-small hover:text-[var(--amber)]">
              WhatsApp {site.phoneDisplay}
            </a>
            <a href={`mailto:${site.email}`} className="t-small hover:text-[var(--amber)]" style={{ wordBreak: "break-all" }}>
              {site.email}
            </a>
            <span className="t-small" style={{ color: "var(--muted)" }}>
              {site.city}
            </span>
            <CalButton className="btn btn-primary btn-sm mt-2">Solicitar diagnóstico</CalButton>
          </div>
        </div>

        <div className="t-label-sm mt-16 flex flex-wrap items-center justify-between gap-3 pt-6" style={{ borderTop: "1px solid var(--border)", color: "var(--muted)", textTransform: "none", letterSpacing: "0.02em" }}>
          <p>
          ...
          <p className="flex items-center gap-2">
            <span className="inline-block h-1.5 w-1.5 rounded-full" style={{ background: "var(--amber)" }} />
            Feito em São Paulo
          </p>
        </div>
      </div>
