...
function Row() {
  return (
    <>
      {logos.map((l, i) => (
        <span key={`${l.alt}-${i}`} className="mx-8 flex shrink-0 items-center gap-2.5" style={{ height: 32, opacity: 0.6 }} title={l.alt}>
          <Image
            src={l.src}
            alt={l.alt}
            width={120}
          ...
          />
          {"label" in l && l.label && (
            <span className="text-[15px] font-semibold" style={{ fontFamily: "var(--font-display)", letterSpacing: "-0.01em" }}>
              {l.label}
            </span>
          )}
        </span>
